...
| `renderer/orion_golden_render/visual.py` | плитки метрик, боковые панели, таблицы выдачи |
| `renderer/orion_golden_render/executive.py` | резюме, матрица рисков, обзор профиля |
| `renderer/fonts/` | Inter (SIL OFL) — им и меряют, и рисуют (ADR-0006) |
...
```bash
npx tsx scripts/run-orion-deck-sections-report72.ts   # 19 ворот, код 1 при провале
python3 renderer/smoke_deck_raster_layout.py          # растр, шкала кеглей, гарнитура в PDF
```

Правки вёрстки проверяются этими двумя командами, а не глазами: типографическая
шкала закрыта (ADR-0008), выброшенное содержимое — CRITICAL (ADR-0007).
