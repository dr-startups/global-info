...
## T2 — как сделано (03.09.2026)

**Второе чтение того же оборота.** Выдача — в снимках, AI-ответ — в
`serp_features` истории позиций (`show_serp_features: 1`). Оба чтения идут одним оборотом тика:
разнеси их — AI-ответы доехали бы следующим, а прогон уже ушёл бы к слиянию. Ответ сохраняется в
`responseJson.positions` строки задачи и пересобирается на каждом обороте, как и снимки.

**Ключ истории позиций — `дата:идентификаторПроекта:индексРегиона`**, а не `дата:позиция:
индексРегиона`, как у снимка. Формы неотличимы на глаз; перепутать значит принять номер проекта
за место в выдаче. `parsePositionsKey` отдельный от `parseSnapshotKey`, обе формы под тестом.

**Тело ответа и его источники — разные строки.** Дека (`fragment-builders/knowledge-ai.ts`)
различает их наличием публичного адреса: `isAnswerBody` печатает текст с подписью, `isSourceRef` —
строку «Источник: …». Поэтому тело идёт **без** `url`, источники — по строке на ссылку без
собственного текста. Подпись под ответом менять не пришлось: `answerCaption` смотрит на провайдера
наблюдения, и `topvisor-yandex` даёт «Ответ поискового ИИ Яндекса, зафиксированный в выдаче» —
ровно то, чем Алиса в выдаче и является (официальный `/v2/gen/search` даёт другую подпись, и это
различение сохранено). **Клиентский текст не изменился, `DECK_CONTENT_VERSION` не двигается.**

**Ответ без единой ссылки в отчёт не идёт** — утверждение, которое некуда проследить. Пустота
названа: `state.aiAbsentQueries` перечисляет запросы без ответа по регионам.

**Готовый ответ Google и панель знаний — один вид `knowledgePanel`.** Различает их только
`rawMetadataSafe.surface`. Поэтому предикат режима судит **элемент**, а не вид
(`surfaceItemAllowedInMode`): фильтр по виду выключил бы в режиме `topvisor` панель знаний молча.
Прежний `surfaceKindAllowedInMode` заменён — один ответ на вопрос «идёт ли поверхность в отчёт».

**Берётся `aiOverview`, а не `aiOverviewPreview`** — превью обрезано до пары сотен знаков, а
платили за полный (`with_ai_overview_full`).

## T3 — как сделано (03.09.2026)

**Подбор идёт после проверки позиций, отдельной фазой `COLLECTING`.** Порядок не случайный:
собранные подсказки попадают в проект фразами, и заказать проверку позиций после них значило бы
оплатить вчетверо больше фраз. Подбор — своя строка задачи (`toolName: "collect"`), свои деньги и
свой отказ: сбой подбора **не роняет** уже оплаченную выдачу, а остаётся предупреждением.

**Имя группы подбора защитой от двойной оплаты быть не может.** Сервис называет её
«DI (город): фраза» — в имени нет поисковика, и подбор одной фразы в Яндексе и Google Москвы даёт
две группы с одинаковым именем; вдобавок сервис пишет фразу без «ё». Якорь — строка задачи: она
заводится до первого платного вызова и дополняется идентификатором группы сразу после каждого.
**Остаточный риск назван:** обрыв ровно между вызовом и записью оплатит один подбор (0,90 ₽)
второй раз.

**Группа подбора обязана остаться выключенной.** Сервис отдаёт её `on: 0`, но полагаться на это
нельзя: включённая группа попала бы в следующую проверку позиций. Проверяем и выключаем сами.

**Состав инструментов Arsenkin отвечает в одном месте.** `arsenkinBudgetForStage` фильтрует свой
список через `arsenkinTools()`, а целевой повтор подсказок отказывает `SUGGEST_NOT_IN_ARSENKIN_TOOLS`
до чтения джобы и до подтверждения оплаты. Это закрывает N1 из ревью T1a: в режиме `topvisor`
кнопка оператора заказала бы второй платный источник подсказок.

**Сколько поверхностей собирать — настройка `TOPVISOR_SUGGEST_REGIONS`** (`config/defaults.ts`),
умолчание — все три поверхности отчёта, 0,90 ₽ за каждую. Цена названа там же, где значение:
сузить список — решение владельца, а не молчаливая экономия.
