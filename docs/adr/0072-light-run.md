...
| Негативный материал без темы каталога (ответ после приёмки, 15.09) | Тоже «негатив найден»: считается любой негативный материал, на экране — число материалов без названия темы | Сайт не называет чистой строку, которую отчёт отмечает негативной. Уровень без темы — `low` (правило `riskFor` для темы без базового уровня); экран результата этапа 4 рисует и случай без тем |
...
3. **Негатив материала:** `resolveItemAdverse` (без прочитанных страниц — список площадок и
   словарь конфига). Запись комплаенса судится только типом риска: санкции, PEP, RCA или
   watchlist — негатив в теме каталога `pep_rca_watchlist` («Санкционные и PEP-списки»); ни
   словарь, ни площадка реестра её негативом не делают — совпадение не подтверждается
   автоматически.
...
5. **`NEGATIVE_FOUND`** — есть хотя бы один негативный материал, с темой или без (решение
   владельца после приёмки 15.09); иначе **`CLEAN`**, уровень `low`.
6. `materialsFound` — все различные негативные материалы; `findingsTotal` — число показанных тем
   (в отчёте находка — тема); уровень проверки — наибольший уровень показанной темы, а без тем —
   `low`; хранится уровнем данных, посетителю печатается ступенью `risk-scale.ts`.
