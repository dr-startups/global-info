...
- [x] **Stage N2 — Google real API:** `GOOGLE_SEARCH_API_KEY` / `GOOGLE_SEARCH_ENGINE_ID`
      ...
      Playwright** — official API only. `external_serp` uses an enum-named provider
      (Serper implemented; no arbitrary URLs from env → SSRF guard) and never falls
      back to mock.
...
- [x] Google/Yandex via **official APIs only**; missing keys → `NOT_CONFIGURED`
      ...
      Custom Search JSON API or a separately-selected external SERP API (Serper).
      Results are **evidence candidates, not verified facts**, and only an
