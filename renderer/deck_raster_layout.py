...
# Геометрия мастер-слайда — из того же листа-модуля, что и у рендерера
# (`orion_golden_render/geometry.py`, шаг 0114): прежняя копия числами
# разошлась с `common.py` при первом же сдвиге нижнего запаса, и растровая
# проверка искала кромку сцены там, где её больше нет. `common.py` отсюда
# импортировать нельзя — этот модуль импортирует `layout_cleeq.py`.
from orion_golden_render.geometry import CONTENT_BOTTOM, MARGIN_X, SLIDE_H, SLIDE_W  # noqa: E402
...
#:
#: Не `CONTENT_BOTTOM`: сцена кончается на 105 000 выше границы контента, её
#: тень — на 45 000 ниже сцены (с шага 0114 это 6 410 200 и 6 455 200), и
