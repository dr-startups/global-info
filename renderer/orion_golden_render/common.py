...
def _trim_dangling_tail(text: str) -> str:
    """Снимает хвост, оборванный резом, — и только его.

    Отчёт 85, стр. 14: «По судебно-криминальной теме найдено 9 материалов, все
    9.» — в деке предложение целое («…все 9 с негативным контекстом»). В списке
    висящих хвостов стоят слова `контекстом`, `негативным`, `криминальным`,
    `санкционным` — они попали туда из обрубленных ярлыков, — и четыре прохода
    подряд отъели конец **законченного** предложения, вернув точку на место.

    Законченная мысль хвостом не бывает: точка, восклицательный и вопросительный
    знак ставит её автор, а не рез. Многоточие — другое дело: его ставит
    поисковик, и оно как раз означает обрыв.
    """
    val = text.strip()
    end = ""
    ...
    if val.endswith("…"):
        ...
        # Точку в конце ставит автор предложения, а не рез: чистить здесь нечего.
        return val
    for _ in range(4):
