...
- сцена рисуется **под** обычным потоком текста и выступает за колонку наружу,
  поэтому ширина строки и ёмкость страницы не меняются ни на EMU. Ёмкость
  объявлена в TS-шаблоне (`maxBulletLinesPerSlide`) и откалибрована по замеру
  рендерера — подвинуть содержимое значит потерять его;
- кегли берутся из объявленной шкалы (`TYPE_SCALE_PT`, ADR-0008);
- текст меряется тем начертанием, которым рисуется (ADR-0006).
"""
