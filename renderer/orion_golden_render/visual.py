...
def _status_tone(status: str) -> tuple[str, "RGBColor"]:
    ...
    # «Не подтверждено», «статус не зафиксирован» — незакрытый вопрос, а не
    # риск и не «всё в порядке»: янтарный. Проверяется до ветки «подтверждено»
    # ниже, иначе отрицание попало бы в неё по подстроке.
    if (
        "вероятн" in s
        or "проверк" in s
        or "требует" in s
...
def _add_search_table(
    ...
    if cols == 2:
        ...
        prop = [0.60, 0.20, 0.20]
    elif cols == 4 and re.search(r"оценк.*совпад", " ".join(map(str, headers)), re.I):
        # Комплаенс-сводка: «База данных | Тип совпадения | Оценка совпадения |
        # Статус проверки». Общая четырёхколоночная ветка отдавала 42 % ширины
        # оценке (в ячейке «78/100») и 18 % статусу, где стоит самая длинная
