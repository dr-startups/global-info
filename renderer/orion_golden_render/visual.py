...
def _render_ai_answers_page(
    ...
    — под ними во всю ширину: сайдбару доставалось 1,6 млн EMU — шесть строк
    по 35 знаков — при бюджете колонки в 660 знаков, и рендерер выбрасывал его
    блоки (прогон DPA-2026-0053, стр. 62). Теперь сайдбар получает ту же
    колонку, что и всякая панель, а тела идут под картинкой в левой колонке;
    что не влезло, дека кладёт на продолжение по мере рендерера.
    """
    ctx.light_bg()
    y = ctx.title(title, 280000, NAVY)
    ...
    if visual:
        img_w = int(CONTENT_W * 0.62) if has_sidebar else CONTENT_W
        side_w = CONTENT_W - img_w - 120000
        column_h = CONTENT_BOTTOM - y - 80000
        # Верхний блок левой колонки: треть — картинка показывает начало
        # ответа, полный текст идёт ниже в той же колонке.
        block_h = min(2_000_000, column_h // 3)
        raw = _resolve_image_bytes(visual)
        if raw:
            iw, ih = img_w, block_h
            if Image is not None:
        ...
        if has_sidebar and side_w > 400000:
            # Сайдбар — на всю колонку, как у любой панели: его бюджет (660
            # знаков) измерен под эту высоту, а не под треть листа.
            _sidebar_analysis(ctx, slide, MARGIN_X + img_w + 120000, y + 60000, side_w, column_h - 60000)
        y = y + block_h + 60000
        if bullets:
            ctx.bullets(
                bullets,
