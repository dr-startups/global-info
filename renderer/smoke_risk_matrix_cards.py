...
def main() -> int:
    ...
    # Лист стал вместительнее на 550 000 EMU (шаг 0114): прежние
    # REGISTRY_CAPACITY худших карточек и сводка влезают целиком, и защита не
    # срабатывает. Защита понижает кегль только последней карточке, и дефицит
    # обязан быть маленьким — целая лишняя карточка уже выбрасывается. Поэтому
    # удлиняется сводка: с 300 знаками прозы требуемая высота 5 449 493 при
    # доступных 5 575 200 — защита срабатывает, содержимое доезжает целиком.
    stepped = [
        {
    ...
    ] + [card(4, f"{SUMMARY_DETAIL} {prose(300)}")]
