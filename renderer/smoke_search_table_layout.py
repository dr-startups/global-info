...
#: Потолок вводного абзаца страницы выдачи и отбивка под ним (`slides.py`,
#: ветка `orion_golden_search_table`). Ёмкость выводится от **объявленного**
#: потолка, а не от фактической высоты двух предложений.
INTRO_MAX_H = 900_000
INTRO_GAP = 40_000
...
def main() -> int:
    ...
    # потолок вводного абзаца (900 000) плюс отбивка. Считать от фактического
    # верха нельзя — он зависит от длины двух предложений и замолчит ровно
    # тогда, когда нужен.
    #
    ...
    # Второй свидетель: настоящая страница с длинным абзацем. Её верх обязан
    # быть не ниже объявленного потолка — иначе 900 000 не потолок.
    intro = " ".join(
        [
            "Показана выдача Яндекса по запросу о проверяемом лице: собранные позиции "
            "перечислены строками таблицы ниже, а полный адрес каждого материала "
            "напечатан отдельной полосой под своей строкой результата.",
            "Материалы повышенного внимания отмечены в колонке оценки, и по каждому из "
            "них рядом со строкой стоит адрес, по которому материал открывается без "
            "дополнительного поиска в интернете и без обращения к оператору отчёта.",
        ]
    )
