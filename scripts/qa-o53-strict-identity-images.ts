/**
 * Manual QA — O5.3 strict identity + autocomplete exposure + image thumbnails.
 *
 * Run: npx tsx scripts/qa-o53-strict-identity-images.ts [caseId]
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-o53-strict-identity-images");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-o53-strict-identity-images" };
...
async function downloadArtifact(url: string, dest: string): Promise<boolean> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-o53-strict-identity-images" } });
...
async function main() {
  ...
  for (const agent of [
    ...
    try {
      const run = await api(`/cases/${caseId}/agents/${agent}/run`, { method: "POST", body: "{}" });
      console.log(`Agent ${agent}: ${run.status ?? "OK"}`);
    } catch (e) {
