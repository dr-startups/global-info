/**
 * Manual QA artifact generator for R3.2b provider diagnostics.
 *
 * Run:
 *   npx tsx scripts/qa-r3-2b-provider-health.ts [caseId]
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-r3-2b-provider-health");
const PAGES_OUT = join(OUT, "pages-pdf");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-r3-2b-provider-health" };
...
async function downloadArtifact(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-r3-2b-provider-health" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.R32B_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";

  console.log(`R3.2b QA artifacts — case ${caseId}`);
  ...
  const focusedPages = [13, 14, 27, 51, 52, 62, toPage];
