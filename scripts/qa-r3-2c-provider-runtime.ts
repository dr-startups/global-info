/**
 * Manual QA artifact generator for R3.2c provider runtime strategy.
 *
 * Run:
 *   npx tsx scripts/qa-r3-2c-provider-runtime.ts [caseId]
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-r3-2c-provider-runtime");
const PAGES_OUT = join(OUT, "pages-pdf");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-r3-2c-provider-runtime" };
...
async function downloadArtifact(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-r3-2c-provider-runtime" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.R32C_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";

  console.log(`R3.2c QA artifacts — case ${caseId}`);
  ...
  if (pdftoppm.status !== 0) {
    ...
    if (fitzExport.status !== 0) {
      console.warn(pdftoppm.stdout || "");
      console.warn(pdftoppm.stderr || "");
      console.warn(fitzExport.stdout || "");
      console.warn(fitzExport.stderr || "");
      throw new Error("page PNG export failed (pdftoppm + fitz)");
    }
  }

  const diagnosticsPage = toPage >= 63 ? 63 : toPage;
  const focusedPages = [10, 13, 14, 27, 51, 52, 62, diagnosticsPage];
