/**
 * Manual QA artifact generator for R3.3 entity filtering.
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-r3-3-entity-filtering");
const PAGES_OUT = join(OUT, "pages-pdf");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-r3-3-entity-filtering" };
...
async function downloadArtifact(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-r3-3-entity-filtering" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.R33_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";
  console.log(`R3.3 QA artifacts — case ${caseId}`);
  ...
  const focusedPages = [8, 13, 14, 17, 20, 24, 27, 29, 32, 34, 36, 51, 52, 62, diagnosticsPage];
