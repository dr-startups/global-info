/**
 * Manual QA artifact generator for R3.4 ORION-like evidence appendix expansion.
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-r3-4-evidence-appendix");
const PAGES_OUT = join(OUT, "pages-pdf");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-r3-4-evidence-appendix" };
...
async function downloadArtifact(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-r3-4-evidence-appendix" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.R34_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";
  console.log(`R3.4 QA artifacts — case ${caseId}`);
  ...
  const r34Pages = [63, 64, 65, 66, 67, 68, 69, 70, 71, 72];
  const focusedPages = Array.from(
    new Set([8, 13, 14, 17, 20, 24, 27, 29, 32, 34, 36, 51, 52, 62, ...r34Pages, diagnosticsPage])
  );
