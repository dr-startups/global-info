/**
 * Manual QA artifact generator for R3.6 production mode / release gate.
 *
 * Renders BOTH internal (73 pages, diagnostics) and client (72 pages, no
 * diagnostics) artifacts and writes the sanitized client report_json so the
 * release gate can be inspected end to end.
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-r3-6-production-gate");
const PAGES_OUT = join(OUT, "pages-pdf");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-r3-6-production-gate" };
...
async function downloadArtifact(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-r3-6-production-gate" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.R36_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";
  console.log(`R3.6 QA production gate — case ${caseId}`);
  ...
  const focused = Array.from(
    new Set([1, 10, 13, 14, 17, 20, 27, 32, 34, 35, 36, 51, 52, 60, 61, 62, 63, 72, internalSlides])
  );
