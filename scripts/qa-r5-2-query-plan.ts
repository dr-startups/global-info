/**
 * Manual QA artifact generator for R5.2 query plan quality.
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-r5-2-query-plan");
const PAGES_OUT = join(OUT, "pages-pdf");
const CLIENT_PAGES_OUT = join(OUT, "client-pages-pdf");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-r5-2-query-plan" };
...
async function downloadArtifact(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-r5-2-query-plan" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.R52_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";
  console.log(`R5.2 QA query plan — case ${caseId}`);
  ...
  const qpd = (internalJson.queryPlanDiagnostics ?? {}) as Record<string, unknown>;
  console.log(`Internal slides: ${internalSlides}  |  Client slides: ${clientSlides}`);
  console.log(`Query plan total/byRegion: ${qpd.totalQueries ?? "n/a"} / ${JSON.stringify(qpd.byRegion ?? {})}`);
  ...
  const focused = Array.from(new Set([7, 8, 10, 11, 12, 13, 14, 23, 24, 26, 27, 51, 52, 63, 72, 73]));
