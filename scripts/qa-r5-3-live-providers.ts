/**
 * Manual QA artifact generator for R5.3 live provider smoke.
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-r5-3-live-provider-smoke");
const PAGES_OUT = join(OUT, "pages-pdf");
const CLIENT_PAGES_OUT = join(OUT, "client-pages-pdf");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-r5-3-live-providers" };
...
async function downloadArtifact(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-r5-3-live-providers" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.R53_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";
  console.log(`R5.3 QA live provider smoke — case ${caseId}`);
  ...
  const clientPolicyHits = forbidden.filter((t) => JSON.stringify(clientJson).includes(t));
  writeFileSync(
    join(OUT, "client-policy-inspection.json"),
    JSON.stringify(
      {
        totalForbiddenHits: clientPolicyHits.length,
        hits: clientPolicyHits,
        status: clientPolicyHits.length === 0 ? "PASS" : "BLOCKED",
      },
      null,
      2
    )
  ...
  const liveSmokeInspection = {
    realOnly: await runLiveProviderSmoke({ requestedRuntimeMode: "real_only", allowLiveCalls: false }),
    realFirstWithFallback: await runLiveProviderSmoke({
      requestedRuntimeMode: "real_first_with_fallback",
      allowLiveCalls: false,
    }),
    mockOnly: await runLiveProviderSmoke({ requestedRuntimeMode: "mock_only", allowLiveCalls: false }),
  };
  writeFileSync(
    join(OUT, "live-provider-smoke-inspection.json"),
    JSON.stringify(liveSmokeInspection, null, 2)
  );
