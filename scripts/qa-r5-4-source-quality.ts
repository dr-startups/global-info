/**
 * Manual QA artifact generator for R5.4 source quality tuning.
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-r5-4-source-quality");
const PAGES_OUT = join(OUT, "pages-pdf");
const CLIENT_PAGES_OUT = join(OUT, "client-pages-pdf");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-r5-4-source-quality" };
...
async function downloadArtifact(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-r5-4-source-quality" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.R54_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";
  console.log(`R5.4 QA source quality — case ${caseId}`);
  ...
  const focused = [8, 13, 14, 20, 24, 27, 52, 53, 54, 55, 56, 57, 58, 59, 60, 63, 72, 73];
