/**
 * Manual QA artifact generator for R6.2 full visual polish pass.
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-r6-2-full-visual-polish");
const PAGES_OUT = join(OUT, "pages-pdf");
const CLIENT_PAGES_OUT = join(OUT, "client-pages-pdf");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-r6-2-visual-polish" };
...
async function downloadArtifact(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-r6-2-visual-polish" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.R62_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";
  console.log(`R6.2 QA visual polish — case ${caseId}`);
