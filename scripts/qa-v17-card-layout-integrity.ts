/**
 * Manual QA — Report v17 card layout integrity (Tomilin case).
 *
 * Run: npx tsx scripts/qa-v17-card-layout-integrity.ts [caseId]
 */
...
const OUT = join(process.cwd(), "storage/digital-profile/qa-v17-card-layout-integrity");
const H = { "Content-Type": "application/json", "x-actor-id": "qa-v17-card-layout-integrity" };
...
async function downloadArtifact(url: string, dest: string): Promise<boolean> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "qa-v17-card-layout-integrity" } });
...
async function main() {
  ...
  const caseId = process.argv[2] ?? process.env.V17_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";

  console.log(`Report v17 card layout QA — case ${caseId}`);
  ...
  console.log(`report-v17: slides=${render.slideCount ?? "?"}`);

  for (const ext of ["pptx", "pdf"] as const) {
    const url = render[`${ext}DownloadUrl`] as string | undefined;
    if (url) {
      await downloadArtifact(url, join(OUT, `report-v17-ru-internal-draft.${ext}`));
    }
  }
  ...
  const pdftoppm = spawnSync(
    "pdftoppm",
    ["-png", "-f", "8", "-l", "36", pdfPath, join(OUT, "pages", "page")],
    { encoding: "utf-8", cwd: process.cwd() }
  );
  if (pdftoppm.status !== 0 && pdftoppm.stderr) {
    console.warn("pdftoppm page export skipped:", pdftoppm.stderr.slice(0, 200));
  }
    ...
    JSON.stringify(
      {
        ...
        layoutVersion: "v17-card-layout-integrity",
      },
      null,
      2
    )
