...
describe("canonical coverage and visual assets", () => {
  ...
  /**
   * Клиентский текст не зовёт в раздел, которого в деке нет.
   *
   * Жёсткий отказ живёт здесь, а не в отчёте сборки: игла выводится из
   * клиентского имени раздела, «приложени» совпадает с обычным русским словом,
   * и в `issues` ложное срабатывание уронило бы `passed` — то есть приёмку
   * эталона и оплаченный прогон на последнем шаге. У эталона-72 приложение в
   * деке есть, поэтому проверка молчит по построению; кадр с отсутствующим
   * разделом держит юнит `deck-text-does-not-promise-a-missing-section`.
   *
   * Своим подтестом, а не строкой в соседнем: сводка называет имя упавшего
   * подтеста, и провал этой проверки не должен читаться как беда с ассетами.
   */
  itWithAssets("клиентский текст не зовёт в отсутствующий раздел", () => {
    assert.equal(
      build.assemblyValidation?.checks.noPromisesOfMissingSections,
      true,
      (build.assemblyValidation?.notes ?? [])
        .filter((n) => /отсутствующий раздел/u.test(n))
        .join(" | ")
    );
  });
});
