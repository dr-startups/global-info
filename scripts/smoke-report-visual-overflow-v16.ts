/**
 * Smoke test — Report v16 visual overflow + media gallery layout.
 *
 * Run: npm run smoke:report-visual-overflow-v16
 */
...
const CASE_ID = process.env.V16_CASE_ID ?? "cmqzz1vbr00d2vdrsrjsgie2g";
const OUT = join(process.cwd(), "storage/digital-profile/smoke-v16-visual-overflow");
const H = { "Content-Type": "application/json", "x-actor-id": "smoke-v16-visual-overflow" };
...
async function download(url: string, dest: string): Promise<void> {
  ...
  const res = await fetch(full, { headers: { "x-actor-id": "smoke-v16-visual-overflow" } });
...
async function main() {
  ...
  console.log(`Report v16 visual overflow smoke — case ${CASE_ID}\n`);
  ...
  check("PPTX v16 layout + integrity inspection", inspect.status === 0, `exit ${inspect.status}`);
    ...
    JSON.stringify(
      {
        ...
        layoutSmoke: "v16-visual-overflow",
