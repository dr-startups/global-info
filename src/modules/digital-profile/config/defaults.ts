...
export const BOOLEAN_DEFAULTS = {
   ...
   * ключ нужен только облаку.
   *
   * Единственная строка списка, к которой оговорка выше про «без ключа в сеть
   * не ходит» не относится: адрес облака задан умолчанием, и без ключа запрос
   * всё равно уходит (замерено). Безусловно требовать ключ нельзя — это
   * объявило бы ненастроенным работающий `yente`, который отвечает анонимно;
   * **условно — можно и, вероятно, нужно**: ключ обязателен ровно тогда, когда
   * адрес равен облачному умолчанию. Это отдельный пункт описи, а не запрет.
   */
