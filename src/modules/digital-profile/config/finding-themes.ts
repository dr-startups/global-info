 ...
 *
 * Семейство записано двумя половинами — учреждением и судимостью. Склейка
 * `COURT_WORD_FORMS` ниже прежняя знак в знак: словарь негатива и тема
 * `criminal_legal` от разделения не меняются. Половины нужны маске признаков
 * субъекта, и почему — сказано у каждой.
 */

/**
 * Учреждение и профессия: слова, которыми описывают место работы и должность.
 *
 * Половина названа отдельно не ради красоты. Признак субъекта, написанный
 * словом этого семейства («Арбитражный суд Краснодарского края»), выводит из
 * улик **всё** семейство: иначе «суд» из словаря вышел бы, а «судья» осталась,
 * и досье председателя суда снова оказалось бы криминальным материалом
 * (`config/subject-context-words.ts`). Дальше семейства маска не идёт: слово
 * должно стоять в самом признаке или быть формой его слова.
 */
export const COURT_INSTITUTION_FORMS =
  "суд(?:а|е|у|ы|ов|ом|ам|ах|ами|ей)?(?!\\p{L})|судебн|судь[яеиюё]|судопроизводств";

/**
 * Судимость и тяжба: событие с человеком, а не название учреждения.
 *
 * В маску признаков не попадает никогда — ни при каком якоре. «С Егорова снята
 * судимость» остаётся уликой у судьи ровно так же, как у всех остальных.
 */
const CONVICTION_FORMS = "судим(?:ост|[аоые])|суд(?:и|я)(?:тся|ться|л[аи]?с[ья])";

const COURT_WORD_FORMS = `${COURT_INSTITUTION_FORMS}|${CONVICTION_FORMS}`;
  ...
  return {
    ...
    themes: [
      ...
      },
      {
        ...
        accusing: false,
        /*
         * `полит(?!ехн)` — «политехнический институт» публичной ролью не
         * является: образование Шойгу стояло под этой темой на живом прогоне
         * 20.09.2026. Тот же класс, что «ФСБР» у оборонного контура.
         *
         * Просмотр назад снимает отраслевые сочетания: «денежно-кредитная
         * политика центрального банка» — это отрасль, а не публичная роль
         * человека. Цена названа: материал о налоговой или кадровой политике
         * теряет совпадение по слову «политика», но не по словам
         * «правительство», «министр» и «парламент», которые в словаре рядом.
         */
        keywords:
          "(?<!(?:денежно-кредитн|кредитно-денежн|учётн|учетн|ценов|кадров|тарифн|налогов|инвестиционн|амортизационн|дивидендн|социальн)[а-яё]{0,3}\\s)полит(?!ехн)" +
          "|politic|депутат|парти|выбор|electoral|минист|правительств|govern|парламент|parliament",
        flags: "iu",
        baseRisk: "medium",
        recommendedAction:
