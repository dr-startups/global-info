...
/**
 * Выгрузка контактов, а не фраза (шаг 0130).
 *
 * Стр. 70 отчёта Мордашова 20.09.2026 цитировала под темой «оборонный контур»
 * строку alestech.ru: «А. Офис в Санкт-Петербурге: 192012, Россия,
 * г. Санкт-Петербург, пр. Обуховской Обороны, д. 112, к. 2, лит.» Машинной
 * выгрузкой она не считалась: доля числовых слов мала, слова настоящие.
 *
 * Признак — почтовый индекс (шесть цифр подряд) **или** три и более адресных
 * сокращения. Одного сокращения мало: «Офис компании на ул. Мира» — обычная
 * фраза; трёх подряд в живой речи не бывает.
 */
const POSTAL_INDEX_RE = /(?<!\d)\d{6}(?!\d)/u;
const ADDRESS_ABBR_RE =
  /(?<![\p{L}])(?:г|ул|пр|просп|пер|наб|ш|д|к|корп|стр|лит|оф|кв|пом|эт)\.\s/giu;
const MIN_ADDRESS_ABBR = 3;

export function looksLikePostalAddress(text: string | null | undefined): boolean {
  const value = String(text ?? "");
  if (!value.trim()) return false;
  const abbr = value.match(ADDRESS_ABBR_RE)?.length ?? 0;
  if (abbr >= MIN_ADDRESS_ABBR) return true;
  return POSTAL_INDEX_RE.test(value) && abbr > 0;
}

/**
 * Подпись к фотографии в начале строки (шаг 0130).
 *
 * Стр. 15 отчёта Мордашова: «Фото: RuNews24 На ПМЭФ-2026 владелец
 * „Северстали“ Алексей Мордашов заявил…». Кредит **не отрезается**: где он
 * кончается, снаружи не видно — «Фото: Алексей Мордашов На фоне завода»
 * режется в другом месте, чем «Фото: RuNews24 На ПМЭФ-2026», и ошибка здесь
 * меняет слова источника. У страницы есть другие предложения.
 *
 * Двоеточие обязательно: «На фото с ПМЭФ Мордашов стоит рядом…» — фраза.
 */
const PHOTO_CREDIT_RE =
  /^\s*(?:фото|фотография|фотограф|photo|изображение|иллюстрация|инфографика|скриншот|кадр)\s*:/iu;

export function looksLikePhotoCredit(text: string | null | undefined): boolean {
  return PHOTO_CREDIT_RE.test(String(text ?? ""));
}
