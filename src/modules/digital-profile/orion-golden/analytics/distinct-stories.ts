...
export function titleFingerprint(title: string): string {
  // Ударения Википедии и пробелы перед знаками — разметка источника, не слова:
  // печать их снимает (`quoteBody`, шаг 0109), и отпечаток обязан снять их той
  ...
  return withoutSourceMarkup(String(title ?? ""))
    ...
    .replace(/\s+/gu, " ")
    .trim()
    // Хвостовая пунктуация — не сюжет (шаг 0122): зеркальные цитаты «…„Единая
    // Россия“.» и «…„Единая Россия“ в 2009—2021 годах.» различались точкой
    // ровно там, где одна продолжается, и проверка «одна начинает другую»
    // не срабатывала — факт печатался дважды.
    .replace(/[.!?;:,]+$/u, "")
    .toLowerCase();
}
