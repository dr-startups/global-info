 ...
 * нежелательного.
 *
 * Два предложения, и это не стилистика: оговорка о неполноте — часть
 * утверждения, а не украшение, поэтому она стоит рядом с ним, а не там, где
 * для неё нашлось место. Строку покрытия и слова о темах собирает один
 * помощник, а не два, — и здесь, рядом с местом, где эти числа родились.
 *
...
export function linkReadingThemesIntro(input: {
  ...
  unread: number;
  /**
   * Сумма публикаций по темам таблицы — то есть её собственная база.
   *
   * Не равна прочитанному: в тему попадает страница, чью принадлежность
   * проверяемому лицу подтвердил её текст. На стр. 20 отчёта Абрамовича
   * 20.09.2026 прочитано было 82, а в таблице стояло 9, и шапка обещала
   * «каждая прочитанная отнесена к теме». Не названа — страница говорит то,
   * что говорила прежде (легаси-вход).
   */
  themedTotal?: number;
}): string {
  const { report, adverseTotal, topN } = input;
  /*
   * Расхождение базы таблицы с прочитанным — называется словами.
   *
   * Причина одна и она не техническая: тему материалу даёт его текст, а текст
   * читается только у страницы, про которую известно, что она о субъекте.
  ...
  const themedClause =
    themed !== undefined && report && themed < report.read
      ? `в таблицу вошли ${themed} ${pluralRu(themed, "публикация", "публикации", "публикаций")}, ` +
        "чью принадлежность проверяемому лицу подтвердил текст страницы, " +
        `нежелательных среди них: ${adverseTotal}.`
      : null;
