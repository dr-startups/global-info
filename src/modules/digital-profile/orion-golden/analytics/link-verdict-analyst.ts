...
/**
 * v4 (шаг 0118): фрагмент принадлежности — отдельным полем `anchorQuote`, в
 * `quotes` только основания вывода. В v3 правило 1a ставило его первой из трёх
 * цитат, и он вытеснял основания: лид биографии впереди фразы о партии.
 * Решения переиспользуются по делу и схеме, версия не сверяется — прежние
 * дела не перечитываются, новый промпт работает на следующем сборе.
 */
export const LINK_VERDICT_PROMPT_VERSION = "link-verdict-prompt-v4" as const;
...
Верни СТРОГО JSON:
{
  ...
  "anchorQuote": {"text": "<дословный фрагмент страницы, где рядом с именем стоит признак лица, 10-400 знаков; если признака нет — не указывай>"},
  "quotes": [{"text": "<дословная цитата со страницы, 10-400 знаков — основание tone и theme>"}],
...
1a. Если в данных лица есть признаки (anchors) — место работы, должность, дата рождения, ИНН, сайт, — то "subject" ставится, только когда хотя бы один из них есть в тексте страницы рядом с именем; процитируй этот фрагмент в anchorQuote, а не в quotes. Признака нет — "unclear", даже если имя совпало полностью.
...
5. quotes — только дословные фрагменты из данного текста, и только основания вывода: фразы, из-за которых tone и theme именно такие. Биографический лид, дату рождения и фрагмент принадлежности в quotes не повторяй — им место в anchorQuote. Если tone="adverse", хотя бы одна цитата обязательна: вывод должен опираться на текст, а не на общее впечатление.
...
/** Фрагмент принадлежности из ответа модели — той же меры, что и цитата. */
function anchorQuoteOf(raw: unknown): { text: string } | undefined {
  const text =
    raw && typeof raw === "object" ? String((raw as { text?: unknown }).text ?? "").trim() : "";
  return text.length >= 10 ? { text: text.slice(0, 400) } : undefined;
}
...
export async function analyzeLinkPage(
  ...
  const parsed = LinkVerdictSchema.safeParse({
    ...
    quotes: Array.isArray(body.quotes)
      ...
      : [],
    anchorQuote: anchorQuoteOf(body.anchorQuote),
    publishedAt:
