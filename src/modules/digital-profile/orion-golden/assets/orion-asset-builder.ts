...
export async function buildOrionGoldenAssets(input: {
  ...
  const imageAssets: ReportAssetV1[] = input.ctx.searchSurfaces
    .filter((s) => isImageSurface(s))
    .slice(0, 24)
    .map((row, idx) => ({
      assetRef: `r10-img-${idx + 1}`,
      kind: "image_grid" as const,
      title: String(row.title ?? row.domain ?? "Изображение"),
      caption: String(row.snippet ?? "").slice(0, 120),
      status: row.imageUrl || row.thumbnailUrl ? ("ready" as const) : ("missing" as const),
