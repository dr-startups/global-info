...
export function withoutSourceMarkup(text: string): string {
  return (
    text
      ...
      // Порядковое, слипшееся со следующим словом (шаг 0126): ответ Алисы
      // пришёл как «в конце 1980-хгодов», «в середине 1990-хгодов». Правило
      // выше сюда не достаёт — между цифрой и словом стоит дефис с суффиксом.
      // Суффиксы перечислены поимённо и коротким списком: «2-годичный» и
      // «5-минутный» — слова, а не порядковые, и разрывать их нельзя.
      .replace(/(\d-(?:х|е|й|ых|ые))(?=[а-яё]{3})/gu, "$1 ")
      // Те же потерянные пробелы других форм (шаг 0128): слово и год
      // («в рейтинге Forbes2026 года»), предлог и сумма («около$37 млрд»),
      // римская цифра и слово («Крестом Признания IIIстепени»). Год ограничен
      // диапазоном 1900–2099 — иначе пробел разорвал бы идентификатор
      // («afNUb1014»), а римская цифра судится только по русскому хвосту.
      .replace(/(\p{L}{3})(?=(?:19|20)\d{2}(?!\d))/gu, "$1 ")
      .replace(/([а-яё])(?=[$€£]\d)/gu, "$1 ")
