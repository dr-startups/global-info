...
export const CompositeObservationRowSchema = z.object({
   ...
   *
   * Слияние сбора сводит два чтения одной выдачи в одно наблюдение, и `rank`
   * несёт только победивший номер. Второй нужен листу, чтобы объяснить пропуск
   * номера в таблице названной причиной, а не общей оговоркой: «позиция 17
   * занята материалом, показанным выше под другим номером» прослеживается до
   * этого поля.
   *
   * Необязательное, версия контракта не двигается: наборы, снятые до его
   * проводки, разбираются как прежде и такой причины не называют вовсе.
   */
  ranksByProvider: z.record(z.number().int().positive()).optional(),
  /**
