...
export const LinkVerdictSchema = z.object({
  ...
  /**
   * Фрагмент принадлежности: имя рядом с признаком субъекта (правило 1a
   * промпта) — отдельно от оснований вывода (шаг 0118).
   *
   * Пока он шёл первой из трёх цитат, он вытеснял основания: лид биографии
   * впереди фразы о партии и о скандалах, и страницы отчёта цитировали лид.
   * Аудит сверяет его с текстом страницы так же, как цитаты. Необязательное:
   * старые артефакты поля не несут, и потребители читают `quotes` как прежде.
   */
  anchorQuote: LinkQuoteSchema.optional(),
  /**
   * Тип источника из закрытого списка (`analytics/source-type.ts`): реестр,
   * СМИ, блог, соцсеть. В таблице выдачи это отдельный столбец — читателю
   * важно, где лежит материал, а не только что он есть.
   */
