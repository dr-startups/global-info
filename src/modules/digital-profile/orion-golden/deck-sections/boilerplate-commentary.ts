...
export function withoutRepeatedBoilerplate(
  ...
  for (const line of src.split("\n")) {
    ...
    if (kept.length === 0 && idTail) {
      /*
       * Строка была одной присказкой с маркером находки.
       *
       * Маркер принадлежит **блоку**, а не строке: `bulletWithFindingId` клеит
       * его к последней строке, какой бы она ни была. Поэтому, если выше в
       * блоке есть текст, присказка снимается, а маркер переезжает на строку
       * выше. С шага 0097 построители отдают блок строками, и присказка стоит
       * последней своей строкой — прежнее «не трогаем» отменяло вычистку
       * целиком: на золотом кейсе снятых повторов стало 0 вместо 3.
       *
       * Блок из одной этой строки по-прежнему не трогается: снять её значит
       * оставить маркер без текста.
       */
      if (droppedHere.length > 0 && lines.length > 0) {
        removed.push(...droppedHere);
        lines[lines.length - 1] = `${lines[lines.length - 1]} ${idTail}`;
        continue;
      }
