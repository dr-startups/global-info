...
/**
 * Кавычка, сразу за которой или перед которой стоит пробел разметки.
 *
 * `« Краснодар »` и `„ Альфа “` — след чужой вёрстки, а не пунктуация
 * источника. Английская пара `“ … ”` считается так же.
 */
const SPACE_INSIDE_QUOTES_RE = /[«„“]\s|\s[»“”]/u;
...
export function clientTextIssues(slides: readonly PrintedSlide[]): string[] {
  ...
  for (const slide of slides) {
    ...
    for (const { field, text } of fieldsOf(slide)) {
      ...
      // 2. Пробел разметки внутри кавычек.
      if (SPACE_INSIDE_QUOTES_RE.test(text)) {
