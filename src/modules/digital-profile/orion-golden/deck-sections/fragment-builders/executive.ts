        ...
        content: {
          ...
          narrative: [
            ...
            `По собранным источникам: ${s.compositeCount} ${pluralRu(
              ...
              .join(", ")}).`,
            /*
             * Обещание проверенной принадлежности — только там, где её
             * проверяли признаком. Без якорей материал отнесён к субъекту по
             * одному совпадению ФИО, и прогон DPA-2026-0049 показал цену этой
             * фразы: 585 «материалов о субъекте» принадлежали четырём людям.
             */
            s.anchoredRun ? "Принадлежность каждого материала к проверяемому лицу проверена." : "",
            s.anchoredRun && (s.unconfirmedSubjectCount ?? 0) > 0
              ? `Из них ${s.unconfirmedSubjectCount} ${pluralRu(
                  s.unconfirmedSubjectCount ?? 0,
                  "совпал",
                  "совпали",
                  "совпали"
                )} с проверяемым лицом только полным именем, без подтверждающего признака.`
              : "",
          ]
            .filter(Boolean)
            .join(" "),
