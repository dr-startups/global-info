...
/** Строка, которой черновик отличается от выпуска в руках у читателя. */
const DRAFT_COVER_LINE = "Черновик для проверки аналитиком — не для передачи клиенту.";

/**
 * Подзаголовок обложки.
 *
 * Черновик собран для проверки аналитиком: в нём стоят материалы, принадлежность
 * которых ещё уточняется, и темы «Требует подтверждения». В чужих руках он
 * неотличим от проверенного документа, поэтому говорит о себе сам.
 *
 * Строка едет текстом в нагрузке, а не новой ручкой рендерера: рендерер прошлой
 * версии напечатает её как обычный подзаголовок обложки, и окна деплоя у правки
 * нет. Подвал страниц при этом не трогается — он зашит в рендерере, и слово
 * «черновик» на каждом листе стоило бы окна деплоя.
 */
function coverNarrative(extras: FragmentExtras): string {
  const confidential = "Конфиденциально. Подготовлено для внутреннего использования клиента.";
  return extras.documentState === "draft"
    ? `${DRAFT_COVER_LINE}\n${confidential}`
    : confidential;
}
  ...
  return {
    slides: [
      makeSlotSlide({
        ...
        content: { narrative: coverNarrative(extras) },
        evidenceRefs: [],
        findingIds: [],
      }),
