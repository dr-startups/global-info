...
export function buildImagesFragment(
  ...
  const slides = slots.map((slot, i) => {
     ...
     *
     * Обвинять материалом, чья принадлежность не подтверждена, нельзя, но и
     * молча снять рамку нельзя тоже: читатель увидел бы чистую сетку там, где
     * плитки ведут на негатив об однофамильцах. Рамки у них нет, объяснения им
     * не положены — остаётся счёт с причиной.
     */
    const wordingOnly = Math.min(sidebar.wordingOnlyRows.length, shownOnGrid);
    const wordingNote = wordingOnly
      ? wordingOnly === 1
        ? " Ещё 1 изображение ведёт на негативный источник, но принадлежность материала не подтверждена — рамкой оно не выделено."
        : ` Ещё ${wordingOnly} ${pluralRu(wordingOnly, "изображение", "изображения", "изображений")} ведут на негативные источники, но принадлежность материалов не подтверждена — рамкой они не выделены.`
      : "";
    const counted = sidebar.explanations.length
      ? `Изображения на этой странице: ${shownOnGrid}; выделено красным (ведут на негативные источники): ${adverseOnGrid}.${wordingNote}`
