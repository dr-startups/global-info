...
export function relatedCompositionBlocks(input: {
  ...
  return {
    ...
    /*
     * «Негатива нет» говорится один раз на листе (шаг 0135).
     *
     * Стр. 33, 34, 46, 47, 48 и 66 отчёта Мордашова 20.09.2026 печатали этот
     * факт трижды разными словами, а стр. 66 из-за третьей строки спорила
     * сама с собой. Числа состава стоят в `whatWasFound` — там же и ответ; эта
     * строка говорит, зачем читателю блок, и о негативе молчит, пока его нет.
     */
    whyItMatters: clampClientText(
      adverseShown > 0
        ...
        : "Связанные запросы поисковик предлагает набрать следующими: по ним видно, что читатель станет искать о субъекте дальше.",
      320
    ),
    ...
    // Строка состояния печатается, только когда ей есть что сказать: негатив
    // на листе или негатив о другом лице. Пустого листа она не комментирует —
    // об этом уже сказала строка состава.
    statusNote:
      adverseShown > 0
