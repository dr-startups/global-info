...
export function panelStatusLine(input: {
  ...
  // Негатива нет ни на панели, ни в наборе, и чужих строк тоже нет: об этом
  // уже сказала строка состава числами (шаг 0135). Второй раз теми же словами
  // — это и есть ощущение наполнителя на шести листах подряд.
  return "";
}
...
export function highlightPhrase(input: {
  ...
  if (!read) {
    ...
    /*
     * Фраза о материале, текст которого не проверялся, написана так, как её
     * написал бы аналитик: сначала сам заголовок (клиент должен видеть, о чём
     * речь), затем тема и основание — заголовок и описание в выдаче, — и
     * честная оговорка, что полный текст не проверялся. Прежняя редакция
     * («страница не читалась в этом прогоне, оценка по заголовку выдачи»)
     * заголовка не печатала и читалась как вывод без основания; владелец
     * назвал это подрывом доверия (04.09.2026). Оговорка не уступает никогда:
     * материал, который не открывали, не должен выглядеть проверенным.
     */
    /*
     * Заголовок — тоже слова источника, и он берётся только из улики.
     *
     * У строки без улики заголовок ничем не подтверждён: в `evidenceRefs`
     * слайда такая строка не попадает, и любое имя издания внутри заголовка
     * («Закон.ru: судью назначили…») ворота читают как названный, но
     * невыводимый источник — прогон DPA-2026-0053 из-за этого не собрал деку
     * вовсе. Без заголовка фраза называет рубрику: своё слово вместо чужого,
     * которое нечем подтвердить.
     */
    const rawTitle = e ? String(e.title ?? "").trim() : "";
    // Заголовок-адрес («https://rupep.org/…») заголовком не является: его и так
    ...
    // Цитата слов выдачи — только когда слово словаря стоит в описании, а не в
    // самом заголовке: заголовок уже напечатан. Заголовок и описание — два
    // наблюдения, и окно цитаты не пересекает их границу.
    const titleHit = adverseTriggerFragment(String(e?.title ?? ""));
    const trigger = titleHit ? "" : adverseTriggerFragment(String(e?.snippet ?? ""));
    ...
    const ladder = [
      head,
      `${named}${domain ? ` — ${domain}` : ""}: ${attribution}по заголовку и описанию в выдаче; ${textNote}${tail}.`,
      `${named}: ${title ? `отнесено к теме «${rubric}» ` : ""}по выдаче; ${textNote}${tail}.`,
      `${named}; ${textNote}${tail}.`,
      `${rubric}; ${textNote}${tail}.`,
      `${textNote.charAt(0).toUpperCase()}${textNote.slice(1)}.`,
    ];
