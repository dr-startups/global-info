...
function slideText(slide: TracedSlide): string {
  const table = [
    ...
    // Адрес печатается колонкой и приходит сюда со строками. Поле полосы
    // осталось в контракте нагрузки, но живого входа у него нет: сборка
    // отказывает на любом его объявлении. Читаем на случай старого артефакта.
