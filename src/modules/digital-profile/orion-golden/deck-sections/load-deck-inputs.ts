 ...
 *
 * Цена раскладки та же, что уже принята для вердикта: материал опознаётся
 * адресом (без настоящего адреса — парой «домен и заголовок»), и записи одного
 * адреса с по-разному обрезанными заголовками делят решение. Третий ответ на
 * «тот же ли это материал» заводить ради этого нельзя.
 *
...
export function applyLinkVerdictsToEvidence(
  ...
  for (const [key, v] of chosen) {
    ...
    for (const ref of materialRefs.get(key) ?? []) {
       ...
       *
       * Адресные группы несут один адрес по построению ключа, но группу без
       * настоящего адреса держит пара «домен и заголовок», и в ней законно
       * лежат разные адреса — подсказки с псевдоадресом, где хеш свой у
       * каждого запроса. Оценку они делят, цитату нет: утверждение обязано
       * прослеживаться до наблюдения со своим URL.
       */
