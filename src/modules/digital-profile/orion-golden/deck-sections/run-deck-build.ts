...
/*
 * Предел итераций — мера длины цепочки, а не сид. После шага 0080 каждая
 * итерация с потерей двигает хотя бы один блок вперёд, то есть цикл конечен;
 * рендерер на цепочке Википедии прогона «Абрамович» (14.09.2026) терял по
 * одному блоку за итерацию — `[1,10] → [1,5,5] → [1,5,4,1] → [1,5,3,2] →
 * [1,5,3,1,1]`, — и четырёх не хватало. Восемь покрывают цепочку из десяти
 * блоков, переезжающих по одному; цена — до четырёх лишних сборок с мерой на
 * редкой деке (шаг 0082).
 */
