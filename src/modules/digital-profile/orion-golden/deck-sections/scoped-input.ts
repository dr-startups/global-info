...
export type LinkReadRegionCounts = {
  ...
  readOther: number;
  /**
   * Из прочитанных: принадлежность подтверждена только совпадением ФИО.
   *
   * В числитель такая страница не попадает никогда (вето разметки делает её
   * «неясной»), а в знаменателе стояла — и доля выходила тем меньше, чем
   * больше в выдаче полных тёзок. Прежние прогоны поля не несут: там нули.
   */
...
export type MetricSnapshot = {
   ...
   * readOther − readUnconfirmed`: прочитанные минус признанные чужими и минус
   * те, у которых совпало только имя. Ни однофамилец, ни неподтверждённая
   * страница ничего о субъекте не подтверждают и не вправе разбавлять долю —
   * по этому же правилу живут таблица тем, резюме и таблица выдачи. «Вероятно»
   * из знаменателя не выкидывается: принадлежность там подтверждена признаком,
   * пусть и слабым.
   *
  ...
  /**
   * Из «требующих идентификации»: совпало полное имя, а подтверждающего
   * признака нет. Обложка называет это число словами — «требуют
   * идентификации» бывает разным.
   */
  unconfirmedSubjectCount?: number;
  /**
   * Принадлежность материалов проверялась признаком, а не одним именем.
   *
   * Выводится из данных прогона (коды причин разметки), а не из слова-флага:
   * прогон без якорей не вправе обещать проверенную принадлежность.
   */
  anchoredRun?: boolean;
...
export type ScopedEvidenceIndex = Record<
  string,
  {
    ...
    /**
     * Значения, по которым разметка признала материал чужим: чужая дата
     * рождения, чужой ИНН, слова тёзки из профиля. Приложение печатает их
     * рядом с причиной — «о другом лице» без значения читателю нечем
     * проверить.
     */
    subjectConflicts?: string[];
    /**
