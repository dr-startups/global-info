...
export const DECK_TEMPLATE_REGISTRY: Record<DeckTemplateId, DeckTemplateDef> = {
  ...
  "serp-table": {
    ...
    methodologyNote:
      "Полный адрес материала напечатан под своей строкой; оценка присваивается по содержанию материала. «Вероятно» — принадлежность субъекту не подтверждена однозначно.",
    maxBulletsPerSlide: 0,
    /*
     ...
     *   потолок вводного абзаца        + 900 000  (slides.py, ctx.body max_h)
     *   отбивка под абзацем            +  40 000
     *   шапка таблицы                  − 330 200  (26 pt)
     *   бюджет строк                 = 3 610 000 EMU
     *
     ...
     *               = 838 200 EMU
     *   ёмкость = floor(3 610 000 / 838 200) = 4   (5 пар — 4 191 000, не влезают)
     *
     ...
     * (`services/render-telemetry-gate.ts`), и `judgeRenderTelemetry`
     * блокирует выдачу только по `CONTENT_DROPPED_BY_RENDERER` и
     * `RENDER_TELEMETRY_MISSING`. На живом прогоне превышение бюджета — одна
     * строка в `job.warnings`, страница при этом уезжает клиенту нарисованной
     * мимо поля. Число обязано держаться само.
     *
     * Верх таблицы берётся **объявленный**, а не фактический (на эталоне абзац
     * занимает 237 815): выведенная из факта ёмкость зависела бы от длины двух
     * предложений и промахнулась бы на живом прогоне.
     *
