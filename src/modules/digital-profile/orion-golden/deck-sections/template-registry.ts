...
export const DECK_TEMPLATE_REGISTRY: Record<DeckTemplateId, DeckTemplateDef> = {
  ...
  "risk-matrix": {
     ...
     *
     * Четыре — по замеру самого рендерера, а не по прикидке. Под содержимым
     * страницы остаётся 5 025 200 EMU; сводная карточка стоит 1 063 536 плюс
     * зазор 50 000: заголовок 13pt жирным меряется в 224 040, тело из двух
     * абзацев 11pt — в 485 546 (в замере уже сидит запас ×1,18 и отбивка
     * абзаца), и сверху карточка берёт свою страховку ×1,12. Четыре занимают
     * 4 454 144 и оставляют пол-карточки про запас, пять требуют 5 567 144 и
     * не помещаются. Текстом это не лечится: тело из двух абзацев — минимум
     * две нарисованные строки, то есть карточка дешевле 1 063 536 не бывает.
     */
