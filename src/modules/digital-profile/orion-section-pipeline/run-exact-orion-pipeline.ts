...
export interface RunExactOrionPipelineOptions {
  ...
  /**
   * R9.5c — when true, required GPT micro-stages MUST be generated by GPT-5.5.
   * A deterministic result for a required stage becomes blocking unless
   * `allowDeterministicFallback` is also true.
   */
  requireAiAnalysis?: boolean;
  /** R9.5c — permit deterministic fallback for required stages (dev/test only). */
  allowDeterministicFallback?: boolean;
  /** R9.5c — override the set of required GPT micro-stages. */
  requiredAiMicroStages?: string[];
}

export type OrionAiEnforcementStatus =
  | "SKIPPED"
  | "PASS"
  | "PASS_WITH_DETERMINISTIC_FALLBACK"
  | "BLOCKED";

export interface OrionAiEnforcementResult {
  requireAiAnalysis: boolean;
  allowDeterministicFallback: boolean;
  status: OrionAiEnforcementStatus;
...
export async function runExactOrionPipeline(caseId: string, options: RunExactOrionPipelineOptions = {}) {
  ...
  const requiredAiStageSet =
    options.requiredAiMicroStages && options.requiredAiMicroStages.length > 0
      ? new Set(options.requiredAiMicroStages)
      : ORION_V2_GPT_REQUIRED_MICRO_STAGE_SET;
  // Only enforce required stages that actually exist in the current blueprint.
  const presentRequiredStages = microStages
    .map((s) => s.microStageKey)
    .filter((key) => requiredAiStageSet.has(key));
  ...
  for (const stage of microStages.sort((a, b) => a.order - b.order)) {
    ...
    try {
      ...
      const selectedForGpt55 =
        (r93GptEnabled && R93_GPT55_STAGE_KEYS.has(stage.microStageKey)) ||
        (requireAiAnalysis && requiredAiStageSet.has(stage.microStageKey));
      let retries = 0;
  ...
  const analysisByStage = new Map(analyses.map((a) => [a.microStageKey, a]));
  const gptStages = presentRequiredStages.filter(
    (key) => analysisByStage.get(key)?.generatedBy === "gpt-5.5"
  );
  const deterministicRequiredStages = presentRequiredStages.filter(
    (key) => analysisByStage.get(key)?.generatedBy !== "gpt-5.5"
  );
  let aiEnforcementStatus: OrionAiEnforcementStatus;
  let aiEnforcementReason: string | null = null;
  if (!requireAiAnalysis) {
    aiEnforcementStatus = "SKIPPED";
  } else if (deterministicRequiredStages.length === 0) {
    aiEnforcementStatus = "PASS";
  } else if (allowDeterministicFallback) {
    aiEnforcementStatus = "PASS_WITH_DETERMINISTIC_FALLBACK";
  ...
  const aiEnforcement: OrionAiEnforcementResult = {
    ...
    requiredStages: presentRequiredStages,
    gptStages,
