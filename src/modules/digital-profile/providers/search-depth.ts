/**
 * Глубина выдачи на один запрос.
 *
 * Правило одно и записано здесь один раз: **настройка провайдера — это глубина
 * по умолчанию, а не потолок**. Потолок задаёт только сам API.
 *
 * Появилось после разбора живого прогона: аудит просил ТОП-20, а оба провайдера
 * считали глубину как `Math.min(запрошено ?? настройка, настройка)` — то есть
 * молча срезали запрошенное до своего значения по умолчанию (10). Отчёт при
 * этом продолжал называть себя аудитом ТОП-20 и показывал первую страницу
 * выдачи. Ошибка не была видна ни в логах, ни в предупреждениях: провайдер
 * честно возвращал `SUCCESS`, просто вдвое короче.
 */
export function resolveSearchDepth(input: {
  /** Сколько просит вызывающий; `undefined` — «не важно, дай по умолчанию». */
  requested?: number;
  /** Глубина по умолчанию из настроек провайдера. */
  fallback: number;
  /** Предел, дальше которого API не имеет смысла просить. */
  max: number;
}): number {
  const wanted = Number.isFinite(input.requested) && (input.requested as number) > 0
    ? Math.trunc(input.requested as number)
    : Math.trunc(input.fallback);
  const safeMax = Math.max(1, Math.trunc(input.max));
  return Math.max(1, Math.min(wanted, safeMax));
}

/**
 * Провайдеры, у которых глубина сверх умолчания стоит отдельных денег.
 *
 * У Serper органика идёт страницами по десять — глубже одной страницы `/search`
 * не отдаёт вовсе, — и глубина оборачивается лишними платными вызовами:
 * двадцатка стоит двух запросов вместо одного. У Яндекса глубина входит
 * в стоимость запроса: вся глубина приходит одной страницей (`groupsOnPage`),
 * а платится сам запрос, не число результатов в нём.
 */
const DEPTH_IS_BILLED: Record<OrganicSearchProvider, boolean> = {
  serper: true,
  yandex: false,
};

/** Провайдеры органической выдачи, у которых сбор просит глубину. */
export type OrganicSearchProvider = "serper" | "yandex";

/**
 * Сколько результатов просить у провайдера по одному запросу сбора.
 *
 * Ограничение глубины — денежное, а не смысловое, и ответ здесь один на оба
 * условия. Дорогая глубина просится только там, где отчёт её обещает: по
 * запросам об имени субъекта строятся позиционные таблицы ТОП-N, а пробы
 * (негативная, деловая, медийная) в них не попадают вовсе. Бесплатная глубина
 * не экономится: урезанная проба — это вдвое меньший корпус без всякой выгоды.
 *
 * `undefined` означает «глубина по умолчанию провайдера»: у каждого адаптера
 * она своя, и второе число здесь было бы вторым ответом на тот же вопрос.
 */
export function organicSearchDepth(input: {
  provider: OrganicSearchProvider;
  /** Назначение запроса из плана (`subject_lookup`, `adverse_lookup`, …). */
  purpose: string;
  /** Глубина, которую отчёт обещает клиенту таблицей выдачи. */
  auditDepth: number;
}): number | undefined {
  if (!DEPTH_IS_BILLED[input.provider]) return input.auditDepth;
  return input.purpose === "subject_lookup" ? input.auditDepth : undefined;
}
