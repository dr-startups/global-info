...
/**
 * Прочитана ли страница и признана ли она благоприятной — один ответ на отчёт.
 *
 * Вопрос не тот же, что «негативен ли материал»: непрочитанная страница
 * негативом не является, но и благоприятной её никто не признавал. Спрашивают
 * этот предикат там, где решается, может ли **обвиняющая** тема опираться на
 * материал: аналитика — берёт ли тема его в состав, счёт и уровень; дека —
 * цитирует ли она его. Это одно решение о материале, и молчание (страницу не
 * читали) основанием для него быть не может.
 *
 * Порядок сил тот же, что у `resolveRowAdverse`: правка аналитика выше вердикта
 * модели. Человек посмотрел материал и сказал «нежелательный» — значит, на
 * странице что-то есть, и тема материал не теряет; иначе строка выдачи
 * печаталась бы «Нежелательной» по правке аналитика, а тема на соседнем листе
 * того же материала не знала бы вовсе.
 */
export function pageReadAsFavourable(row: {
  tone?: ObservationVerdict["tone"] | null;
  analystDecision?: AnalystDecision | null;
}): boolean {
  if (row.analystDecision === "ADVERSE") return false;
  return row.tone === "neutral" || row.tone === "supportive";
}
