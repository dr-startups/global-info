...
/**
 * Подпись снимка выдачи.
 *
 * Раньше здесь стояло «Синтетический снимок на основе сохранённых результатов
 * API». Слово «синтетический» на картинке с настоящими результатами читается
 * клиентом как «выдумано»: строки, домены и заголовки в снимке — ровно то, что
 * вернул поисковик, и сомнение в них не заслужено. Происхождение при этом не
 * скрывается: справа в подвале снимка печатается источник («Источник: реальные
 * данные Yandex Search API», а для демо-данных — прямо «demo/mock-данные»), и
 * эта строка остаётся главной честной пометкой.
 */
export const SERP_SNAPSHOT_CAPTION = "Результаты поисковой выдачи на момент сбора";
