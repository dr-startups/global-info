 ...
 *
 * Было четыре часа — «заведомо больше любого честного прогона». Такой потолок
 * ограничивает разве что вечное зависание: наблюдённый полный прогон идёт
 * около двадцати минут, то есть в двенадцать раз меньше. На практике это
 * означало, что при залипшем провайдере оператор ждёт полдня, ничего не
 * получая, — а правило продукта говорит обратное: отказ одного источника не
 * обнуляет оплаченный сбор, прогон доходит до `COMPLETED_PARTIAL` и отдаёт
 * собранное.
 *
 * Час — это втрое больше наблюдённого полного прогона и вчетверо больше
 * первой стадии, которая с ADR-0005 идёт по умолчанию одна. Запас на медленный
 * день у провайдера есть, а полудня ожидания больше нет.
 *
 * Отсчёт по-прежнему не сбрасывается при автоматическом возобновлении: иначе
 * ограничения не было бы вовсе — ровно та дыра, из-за которой ручные нажатия
 * продлевали прогон бесконечно.
 */
export const MAX_ENRICHMENT_WAIT_MS = 60 * 60 * 1000;
