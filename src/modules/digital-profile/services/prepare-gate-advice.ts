...
const DETERMINISTIC_GATES = [
  ...
  // Ни один материал не подтверждён признаком субъекта: тот же корпус и те же
  // признаки дадут тот же ответ, а стоит второй заход четырёх стадий модели.
  "SUBJECT_ANCHORS_NO_MATCH",
  "P1_P2_ACCOUNTED",
...
export function prepareGateAdvice(message: string | null | undefined): string | null {
  ...
  switch (gate) {
    ...
    case "SUBJECT_ANCHORS_NO_MATCH":
      return (
        "Ни один собранный материал не подтверждён признаком проверяемого лица — " +
        "совпало только имя. Повтор сборки это не изменит: откройте «Выбор персоны " +
        "субъекта», проверьте, найдены ли признаки в выдаче, и напишите их так, как " +
        "пишет о человеке пресса («АС Краснодарского края» рядом с полным названием), " +
        "затем нажмите «Пересобрать отчёт»."
      );
