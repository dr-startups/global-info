 ...
 *   · клипы — предупреждения, а не блок: `clipped` на текстовом пути это
 *     предсказание меры высоты, а мера завышает требуемую высоту примерно на
 *     треть, и блокировка по ней останавливала бы здоровые прогоны;
