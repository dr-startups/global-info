...
export async function evaluateLegacyRecoveryEligibility(input: {
  ...
  const job = input.job;
    ...
    // узок: повтор стоит полного прохода GPT-copy (§8 ENGINEERING.md), поэтому
    // кнопка положена ровно тем отказам, которые рендером и лечатся. Широкий ключ
    // приглашал бы чинить повтором `PREPARE_INPUT_MISSING` — то самое
    // приглашение потратить время впустую, против которого написан
