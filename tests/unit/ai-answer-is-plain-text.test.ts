...
describe("пробелы, потерянные провайдером, восстанавливаются (шаг 0117)", () => {
  // Ответ Алисы на прогоне 18.09.2026 пришёл без пробелов после чисел и вокруг
  // тире: «Родился 9мая 1967года», «Бондарчук—советский». Слова не трогаются.
  it("П1: между цифрой и русским словом ставится пробел", () => {
    const text = plainAiAnswerText("Родился 9мая 1967года в Москве. Снимался в «86400секунд работы дежурной части милиции» (1988).");
    expect(text).toContain("9 мая 1967 года");
    expect(text).toContain("«86400 секунд работы");
  });

  it("П2: тире между словами получает пробелы, диапазон чисел и латиница не трогаются", () => {
    const text = plainAiAnswerText(
      "Фёдор Бондарчук—советский актёр. «Даун Хаус» (2001)—интерпретация романа. Член совета в 2009—2021 годах. 2х-комнатная квартира, фильм в 3D, сеть 5G."
    );
    expect(text).toContain("Бондарчук — советский");
    expect(text).toContain("(2001) — интерпретация");
    expect(text).toContain("2009—2021");
    expect(text).toContain("2х-комнатная");
