...
describe("перекладка буллетов по мере рендерера", () => {
  ...
  it("чистый вердикт без места даёт пустой план и неизменную сборку", () => {
    // Правка шага 0102: прежде листы несли запас в 4 000 000 EMU, и план был
    // пуст, потому что блоки ездили только вперёд. Теперь запас — повод
    // уплотнить, и пустой план обещан листам, заполненным впритык.
    const slides = [
