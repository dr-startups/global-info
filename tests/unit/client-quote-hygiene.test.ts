...
/**
 * Шаг 0112. Итоговые отчёты 18.09.2026: резюме цитировало навигацию и поля
 * карточек агрегаторов — «Сводка Руководитель Учредитель ИП Связи Факторы
 * риска», «Суды 1 на сумму X XXX ₽ …», «ФИО … Должность … Дата рождения …».
 * Это не высказывание источника, а его интерфейс. Правило — по форме текста,
 * не по списку доменов.
 */
describe("интерфейс карточки — не цитата", () => {
  it("К1: четыре живых примера отвергаются", () => {
    const chrome = [
      "Бондарчук Федор Сергеевич Сводка Руководитель Учредитель ИП Связи Факторы риска",
      "Суды 1 на сумму X XXX ₽ Все За последние 5 лет Имеет судебные дела с компаниями Ответчик 1",
      "ФИО Федор Бондарчук Должность Актер, кинорежиссер, продюсер, телеведущий, клипмейкер Дата рождения 1967-05-09",
      "Бондарчук Фёдор Сергеевич советский и российский актёр, продюсер кино и телевидения Дата рождения: 09.05.1967 Место рождения: г. Москва",
    ];
    for (const text of chrome) expect(pageQuoteForClient(text), text).toBe("");
  });

  it("К2: проза с теми же словами и заголовок с ИНН остаются", () => {
    const prose = [
      "Руководитель компании заявил на собрании, что учредитель вышел из состава участников общества.",
      "Дата рождения указана в паспорте, а место рождения — в свидетельстве, выданном в Москве.",
      "Бизнесмен Дерипаска Олег Владимирович, ИНН 190200291847, зарегистрирован как индивидуальный предприниматель.",
      "Федор Бондарчук снова свободен. Первая жена режиссера, Светлана, после 25 лет брака получила квартиру на Патриарших.",
    ];
    for (const text of prose) expect(pageQuoteForClient(text), text).toBe(text);
  });
});

describe("приклеенная обвязка страницы (шаг 0115)", () => {
  it("К3: текст до склейки «…в МосквеИсточник: Starface.ru.» остаётся, обвязка уходит", () => {
    const q = pageQuoteForClient(
      "В 2019 году режиссёр женился на актрисе Паулине Андреевой, свадьбу сыграли в МосквеИсточник: Starface.ru."
    );
    expect(q).toBe("В 2019 году режиссёр женился на актрисе Паулине Андреевой, свадьбу сыграли в Москве");
  });
});
