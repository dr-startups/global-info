...
describe("многоточие посреди заголовка — тот же обрыв", () => {
  /**
   * Отчёт 75: «Геннадий Тимченко: биография предпринимателя... - Новости Mail»
   * стояло цитатой в резюме для руководства и в матрице рисков. Поисковик режет
   * заголовок и приклеивает подпись издания; признак обрыва ловил многоточие
   * только в конце строки.
   */
  it("обрывок с подписью издания не цитируется", () => {
    for (const raw of [
      "Геннадий Тимченко: биография предпринимателя... - Новости Mail",
      "Геннадий Тимченко: не столько бизнесмен, сколько... - Ведомости",
      "Тимати | биография, фото, карьера... | Дзен",
    ]) {
      expect(quoteForClaim(raw, 220), raw).toBe("");
    }
  });

  it("целое предложение перед обрывом сохраняется", () => {
    expect(quoteForClaim("Алишер Усманов. Из тюрьмы в Форбс. ...", 220)).toBe(
      "Алишер Усманов. Из тюрьмы в Форбс."
    );
  });

  it("тире в целом заголовке не путается с подписью издания", () => {
    const whole = "Тимченко Геннадий Николаевич / «Компания». Биографии";
    expect(quoteForClaim(whole, 220)).toBe(whole);
  });
});
