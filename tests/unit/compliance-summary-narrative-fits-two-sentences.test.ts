/**
 * Нарратив сводной комплаенс-страницы укладывается в то, что рисует рендерер.
 *
 * Шаблон `orion_golden_search_table` печатает над таблицей ровно два полных
 * предложения (`renderer/orion_golden_render/slides.py`), остальное молча
 * отбрасывает. Поэтому третье честное предложение существует в артефакте, в
 * снапшоте и в тексте — и не существует на странице PDF. Проверка стоит на
 * стороне построителя: снапшот пинует его вход, а не отрисованный лист.
 *
...
describe("нарратив сводной комплаенс-страницы", () => {
  it("не превышает двух предложений ни в одной ветви", () => {
