...
const EXTRA_SOURCES = [
  ...
  // Раскрой таблиц по мере рендерера: он решает, сколько строк выдачи стоит
  // на листе, — то есть состав каждой страницы таблицы, её опоры и её
  // счётчики. Основание то же, что у перекладки буллетов рядом.
  "measured-table-fit.ts",
  ...
  // Тело цитаты: как печатаются кавычки внутри цитаты, ударения и пробелы
  // разметки (`quoteBody`, `withoutSourceMarkup`). Правка в нём меняет каждую
  // цитату каждого блока темы, не трогая ни одного построителя: на шаге 0115
  // пробелы внутри «« Единая Россия »» ушли из печати, а отпечаток без этой
  // строки остался прежним — и кэш паков отдал бы старые кавычки.
  "../client/client-quote.ts",
