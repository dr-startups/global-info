...
describe("ИИ-ответ не выдаётся за публикацию", () => {
  ...
  it("ИИ-ответ назван ИИ-ответом", () => {
    const said = lines.find((l) => /в ИИ-ответе поисковой системы/u.test(l));
    expect(said, "ожидалась формулировка про ИИ-ответ").toBeTruthy();
    expect(said!).toMatch(/требует сверки с первоисточником/u);
  });
