...
describe("доказательство названо тем, чем является", () => {
  ...
  it("вопрос из выдачи не подаётся как публикация", () => {
    // Вопрос ищется и в кавычках (цитата внутри темы), и как самостоятельная
    // строка страницы «связанные запросы». Кавычечная форма в золотом кейсе
    // больше не встречается: похожие вопросы не входят в предмет аудита
    // (ТОП-20 выдачи) и темами не подкрепляются — но проверка обязана держать
    // весь текст, а не только ту форму, которая была на момент бага.
    const questions = lines.filter((l) => /«[^»]+\?»/u.test(l) || /\?\s*$/u.test(l));
    expect(questions.length).toBeGreaterThan(0);
    for (const line of questions) {
      const m = line.match(/(.{0,60})(«[^»]+\?»|\?\s*$)/u);
      expect(
        m?.[1] ?? "",
