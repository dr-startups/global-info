...
describe("без решения аналитика фраза прежняя", () => {
  it("непрочитанная строка по-прежнему называет заголовок выдачи основанием", () => {
    ...
    // Формулировка аналитика (04.09.2026): заголовок, тема, основание,
    // оговорка о непроверенном тексте — и адрес.
    expect(phrase.full).toContain("— finansbladet.se: отнесено к теме «Деловой профиль» по заголовку и описанию в выдаче;");
    expect(phrase.full).toContain("текст страницы в этом прогоне не проверялся; материал учтён в находках отчёта.");
    expect(phrase.sidebar).toContain("не проверялся; материал учтён в находках отчёта.");
