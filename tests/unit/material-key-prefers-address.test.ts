...
describe("ключ материала предпочитает адрес", () => {
  ...
  it("адрес читается с меткой отслеживания: ?shem= и без — два ключа", () => {
    // Решение владельца 3A: список меток отслеживания в дереве один
    // (`services/evidence-service.ts`), второго здесь не заводим — цена
    // записана в опись пунктом EL.
    const title = "Глинка Сергей Михайлович";
    expect(
      serpMaterialKey({ url: "https://www.tadviser.ru/index.php/Персона?shem=rimspwouoe,", title })
    ).not.toBe(serpMaterialKey({ url: "https://www.tadviser.ru/index.php/Персона", title }));
  });
});
