...
describe("мера выше арифметики", () => {
  ...
  it("потеря строк без потери блоков тоже двигает последний блок", () => {
    ...
    // До шага 0106 ожидалось [1, 4, 6]: уехавший блок сваливался на полный
    // лист сверх его ёмкости. Теперь переполнение течёт на новый лист.
    expect(plan.get("p29_uae_wikipedia")).toEqual([1, 4, 5, 1]);
  });
