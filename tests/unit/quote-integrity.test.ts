...
describe("вложенные кавычки внутри цитаты", () => {
  /**
   * Прогон 72 дал ложное нарушение: ворота брали первую закрывающую ёлочку —
   * ту, что закрывает кавычки ВНУТРИ цитаты, — и источник за настоящей
   * закрывающей уже не видели.
   */
  it("источник за внешней кавычкой находится", () => {
    const bullet = [
      "«Политические связи / публичная экспозиция»",
      "Найдены материалы о политической и публичной экспозиции субъекта:",
      "«Бизнесмен Алишер Усманов подал судебный иск против банка UBS Europe SE в связи с «политически мотивированными» расследованиями» — источник iz.ru",
      "Всего по теме: 6 материалов.",
    ].join("\n");
    expect(quoteIntegrityProblems(bullet)).toEqual([]);
  });

  it("цитата с вложенными кавычками и без источника всё ещё нарушение", () => {
    const bullet = [
      "«Тема»",
      "Найдены публикации:",
      "«Иск в связи с «политически мотивированными» расследованиями»",
      "Всего по теме: 1 материал.",
    ].join("\n");
    expect(quoteIntegrityProblems(bullet).some((p) => p.startsWith("quote without a source"))).toBe(
      true
