 ...
 *   · клип — громкое предупреждение, но не блок: он означает сработавшую
 *     защиту (понижение кегля, ножницы предложений), а пропавшее содержимое
 *     считается отдельно в `dropped*`; на прочих путях мера к тому же
 *     консервативна, и блокировка по её предсказанию останавливала бы
 *     здоровые прогоны;
