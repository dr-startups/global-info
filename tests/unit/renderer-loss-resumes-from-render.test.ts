 ...
 *
 * Насколько путь дешевле пересборки — вопрос не этого модуля: подготовка
 * переиспользует собранную деку и платит за возобновление один рендер
 * (§8 ENGINEERING.md).
 *
 ...
 * «похоже на рендер». Признак `isRenderFailure` шире этих четырёх кодов: он
 * истинен и при чекпоинте `RENDER` (терминальные патчи подготовки чекпоинт не
 * чистят, и он остаётся от прошлой попытки), и при свободном тексте «render
 * failed» в сообщении. Отказы, случившиеся до рендера, повтором рендера не
 * лечатся: кнопка была бы приглашением к работе, которая не может помочь.
 *
 * Все четыре кода — одного рода: собранное цело, испорчен документ. Обрезанная
 * строка комплаенса (`COMPLIANCE_CARD_CLIPPED`) в их числе, потому что после
 * правки ёмкости страницы тот же самый рендер пройдёт, а платить за стадии GPT
 * заново незачем.
 */
describe("повтор рендера предлагается ровно четырём кодам", () => {
  it.each([
    "RENDER_FAILED",
