...
describe("страниц матрицы столько, сколько нужно", () => {
  ...
  it("при ёмкости в две карточки одинокий хвост остаётся, а не переезжает", () => {
    // Ёмкость задаётся реестром и сегодня равна трём; двойка приходит сюда
  ...
  it("страница не остаётся полупустой, когда подтверждённых мало", () => {
    ...
    // набраться до ёмкости, а не встать на двух карточках. Раскладка целиком —
    // литералом: «первая страница полна, всего семь» переживёт и [3, 1, 1, 1, 1].
    const pages = packRiskMatrixPages(confirmed(1), likely(6));
    expect(pages.map((p) => p.length)).toEqual([3, 2, 2]);
    expect(pages[0]![0]!.subjectMatch).toBe("SUBJECT_MATCH");
  });
});

describe("ёмкость листа выведена из худшей легальной карточки", () => {
  /*
   * Числа здесь литеральные намеренно: остальной файл выражает раскладки через
   * ёмкость и потому переживает любое её значение, а этот блок закрепляет само
   * значение. Три — это не «подобрано под замер»: столько помещается худшей
   * легальной карточки, какую способен выпустить построитель (строка «в чём
   * проблема» на 160 знаков плюс «Что делать» на остатке бюджета 320 — четыре
   * нарисованных строки тела, 1 307 808 EMU при 4 925 200 под сеткой карточек).
   * Четвёртая такая карточка на лист не помещается, и рендерер выбросил бы её
   * с блокирующей потерей. Те же числа держат комментарий реестра и смок
   * `renderer/smoke_risk_matrix_cards.py`.
   */
  it("реестр объявляет три карточки на лист", () => {
    expect(CAPACITY).toBe(3);
  });

  it("четвёртая тема уже стоит второй страницы", () => {
    // Плата за ёмкость 3, названная числом: лишняя страница появляется не «от
    // семи тем», а начиная с четырёх — при ёмкости 4 те же четыре темы лежали
    // на одном листе. Правило хвоста делит их поровну, а не 3 + 1.
    expect(pagesOf(confirmed(3)).map((p) => p.length)).toEqual([3]);
    expect(pagesOf(confirmed(4)).map((p) => p.length)).toEqual([2, 2]);
  });

  it("шесть подтверждённых тем ложатся в две полных страницы", () => {
    expect(pagesOf(confirmed(6)).map((p) => p.length)).toEqual([3, 3]);
  });

  it("десять тем прогона 76 доезжают до листа четырьмя страницами", () => {
    expect(pagesOf(confirmed(10)).map((p) => p.length)).toEqual([3, 3, 2, 2]);
    expect(packRiskMatrixPages(confirmed(7), likely(3)).map((p) => p.length)).toEqual([
      3, 3, 2, 2,
    ]);
  });

  it("состав и порядок страниц золотого кейса не меняются", () => {
    // Доказательство того, что клиентский текст golden не двинулся: и при
    // прежней ёмкости 4 (через перенос хвоста), и при ёмкости 3 (сразу)
    // четыре подтверждённых темы с агрегатом вероятных дают те же две
    // страницы в том же порядке.
    expect(pagesOf(confirmed(4), 55)).toEqual([["c1", "c2", "сводка"], ["c3", "c4"]]);
  });
});
