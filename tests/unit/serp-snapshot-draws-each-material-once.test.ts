...
describe("цена ключа: заголовок сильнее адреса", () => {
  it("одинаковый заголовок при разных адресах сегодня даёт одну строку колонки", () => {
    /*
     * Это **не** желаемое поведение, а измеренная цена ключа `домен|заголовок`:
     * две разные страницы одного издания с одинаковым заголовком (обрезка
     * заголовка провайдером — обычное дело) сводятся в одну, и вторая на
     * картинку не попадает. Отчёт при этом остаётся согласованным — таблица
     * выдачи склеивает их тем же ключом и так же, — но результат поиска
     * пропадает с листа. Чинится это правкой самого ключа, а она двигает
     * таблицы обоих эталонов, поэтому вынесена строкой в бэклог. Тест стоит
     * здесь, чтобы потеря была записана, а не подразумевалась: тот, кто ключ
     * починит, увидит его красным и прочитает эту причину.
     */
    const first = obs({
