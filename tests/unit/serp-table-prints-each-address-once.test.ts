...
describe("печатная таблица выдачи не повторяет адрес", () => {
  ...
  it("адрес с меткой отслеживания и без — один адрес, и повтор виден", () => {
    /*
     * Прежнее решение владельца («метки остаются частью ключа, второго списка
     * не заводим») отменено партией 0042: список один, читателей двое. Ворот
     * обязан видеть повтор — именно два таких соседних ряда владелец и увидел
     * на стр. 41 прогона 91, а приёмка их пропускала.
     */
    const repeats = repeatedSerpTableAddresses([
      tableSlide({ slideKey: "p09", addresses: ["tadviser.ru/p?shem=rimspwouoe,", "tadviser.ru/p"] }),
    ]);
    expect(repeats).toHaveLength(1);
    expect(repeats.join(" ")).toContain("tadviser.ru/p");
  });
