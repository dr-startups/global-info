...
describe("оценка строки берётся у прочитанной страницы", () => {
  ...
  function scopedWithTone(tone: string | undefined, title: string): ScopedFragmentInput {
    ...
    //
    // `adverse` пишет тот же загрузчик, что и тон: он означает «нежелательный
    // вывод, подтверждённый цитатой». Без цитаты нежелательный вывод решения не
    // приносит, и ответ отдаётся словарю.
    if (tone) {
      e.readVerdictTone = tone;
      e.adverse = tone === "adverse";
    }
    return scoped;
  }
