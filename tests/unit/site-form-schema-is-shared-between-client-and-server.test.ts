...
describe("форма проверки", () => {
  it("ошибки у полей — те же, что в ответе 400 сервера", () => {
    ...
    // Сервер при этом отвечает своей строкой — форма ничего не «чинит» за него
    expect(server.inn).toBe("В ИНН 10 или 12 цифр — проверьте номер.");
  });
...
describe("схема одна", () => {
  ...
  it("схема собирается в браузере: ни базы, ни серверных модулей", () => {
    expect(imports(source("src/modules/self-check/schemas.ts")).sort()).toEqual(["zod"]);
  });
});
