...
describe("цитата источника сбалансирована", () => {
  ...
  it("цитата без источника снимается вместе с обещанием над ней (шаг 0092)", () => {
    // Живой блок приложения «Чайки» после 0091: знаки в порядке, а источника у цитаты нет.
    const block =
      "«Внимание по линии безопасности / оборонный контур»\nПринадлежность: не разобрана — совпало полное имя.\nНайдены материалы с акцентом на безопасность и оборонный контур:\n«Новым главой Россотрудничества стал Игорь Чайка, участвовавший в тайных операциях ФСБ за рубежом Владимир Путин назначил сына бывшего»\nВсего по теме: 1 материал, с негативным контекстом — 1. [finding-security_scrutiny-ambiguous-6c2f8af2]";
    const fixed = normalizeQuoteMarks(block);
    expect(fixed).toBe(
      "«Внимание по линии безопасности / оборонный контур»\nПринадлежность: не разобрана — совпало полное имя.\nВсего по теме: 1 материал, с негативным контекстом — 1. [finding-security_scrutiny-ambiguous-6c2f8af2]"
    );
    expect(quoteIntegrityProblems(fixed)).toEqual([]);
    // Блок, называющий источники строкой «Где видно», цитату без атрибуции сохраняет — как и ворота.
    const named = "«Деловой профиль»\nНайдены материалы:\n«Биография Игоря Чайки.»\nГде видно: news.mail.ru.";
    expect(normalizeQuoteMarks(named)).toBe(named);
    // Название темы в первой строке — наше слово, источника у него не бывает.
    expect(normalizeQuoteMarks("«Криминальные / судебные материалы»")).toBe("«Криминальные / судебные материалы»");
  });
