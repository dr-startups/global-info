...
describe("слова резюме из сюжетов", () => {
   ...
   *
   * «Основные основания» перечисляли сюжеты прочитанных страниц, а плитка
   * «Тем риска» рядом считала строки матрицы — рубрики словаря. На живом
   * отчёте 21.08 читатель видел «3 Темы риска» и тут же четыре названия: два
   * счёта одного, на первый взгляд, вопроса (пункт CP). Сущности разные,
   * поэтому и называются по-разному.
   */
  it("итоговая оценка называет сюжеты сюжетами", () => {
    const pack = buildPack();
    expect(pack.overallAssessment.conclusion).toBe(
      `Итоговая оценка: высокий риск. Основные сюжеты в выдаче: ${STOCKHOLM}; ${MALTA}.`
    );
    ...
    // Абзац отдаётся строками (шаг 0098): вывод — первым предложением своей
    // строкой (только он печатается жирным), перечень — следующей, «Ограничения»
    // — подзаголовком без точки. Слова прежние.
    expect(composeClientSummary({ pack }).sections.overallAssessment).toBe(
      [
        "Итоговая оценка: высокий риск.",
        `Основные сюжеты в выдаче: ${STOCKHOLM}; ${MALTA}.`,
        "Ограничения",
  ...
  it("блок сюжета печатается целиком и дословно", () => {
    ...
    /*
     * Блок отдаётся строками (шаг 0097): заголовок своей строкой без точки,
     * дальше по строке на часть — счётная фраза, источники, цитата, доля
     * процитированного, оговорка. Слова те же; прежде части склеивались
     * пробелом, и рендерер печатал блок стеной, а структуру угадывал.
     */
    ...
    expect(themeBlockText(malta.heading, malta.body)).toBe(
      [
        MALTA,
        "По сюжету прочитано 2 публикации, 1 из них нежелательная.",
  ...
  it("следующие проверки называют сюжеты одной строкой", () => {
    const summary = composeClientSummary({ pack: buildPack() });
    // Заголовок блока — своей строкой без точки, пункт — своей строкой (шаг 0097).
    expect(summary.sections.nextSteps).toBe(
      [
        "Следующие проверки",
        `1) Сверить первоисточники и статус материалов по теме «${STOCKHOLM}», «${MALTA}».`,
        "2) Сверить актуальность санкционных и мониторинговых записей по официальным источникам.",
        "3) Подготовить согласованную позицию и единый пакет документов для KYC и партнёрских запросов.",
      ].join("\n")
    );
  });
