...
describe("хвост издания в заголовке", () => {
  ...
  it("Х2: короткий остаток не режется — это не хвост, а часть заголовка", () => {
    /*
     * Прежний пример («Интервью · Матвиенко о санкциях») сам оказался похож на
     * навигацию площадки — два коротких ярлыка через «·», — и с шага 0121 не
     * цитируется вовсе. Смысл проверки прежний: рубрика перед знаком короче
     * двадцати знаков не превращает заголовок в обрубок, — и держит его пример,
     * который навигацией не является.
     */
    const title = "Суд · Потанин обжаловал раздел активов в Лондоне";
    expect(quoteForClaim(title, 220)).toBe(title);
  });
});
