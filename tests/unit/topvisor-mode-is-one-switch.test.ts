...
describe("состав сборщиков в режиме topvisor", () => {
  ...
  it("Arsenkin остаётся ради подсказок Google и «люди также спрашивают»", () => {
    // Позиции и подсказки Яндекса собирает Topvisor; два источника одного и
    // того же означали бы два ответа на один вопрос — и двойную оплату. Но
    // подсказки Google Topvisor не отдаёт, и они остаются за Arsenkin.
    useMode("topvisor", KEYS);
