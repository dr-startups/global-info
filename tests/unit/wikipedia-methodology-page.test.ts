...
describe("рекомендация зависит от результата проверки", () => {
  ...
  it("тёзка не выдаётся за проверяемое лицо", () => {
    ...
    const narrative = slide.content.narrative ?? "";
    // Страница называет способ находки, а не мнимое совпадение заголовка: у
    // «Глинка (дворянский род)» совпала фамилия, а у находки по межъязыковой
    // ссылке заголовок латиницей может не совпасть с нашей транслитерацией.
