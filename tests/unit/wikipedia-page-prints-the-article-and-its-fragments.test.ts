...
describe("дословное начало статьи печатается без знаков ударения (шаг 0113)", () => {
  it("Л1: ударения и пробел перед скобкой из разметки Википедии снимаются, метка «дословно» на месте", () => {
    const lead =
      "Оле\u0301г Влади\u0301мирович Дерипа\u0301ска ( род. 2 января 1968 , Дзержинск ) — российский предприниматель. " +
      "Владелец компании Базовый элемент.";
    const bullet = allBullets({ review: { lead } }).find((b) => b.startsWith("Начало статьи (дословно): "));
    expect(bullet).toBeDefined();
    expect(bullet!).toContain("Олег Владимирович Дерипаска (род. 2 января 1968, Дзержинск) — российский предприниматель.");
    expect(bullet!).not.toMatch(/[\u0300-\u036f]/u);
  });
});
...
describe("о содержании статьи говорит один инструмент", () => {
  it("разбор нашёл негатив — страница не объявляет статью чистой", () => {
    // Словарь заголовков (`getAdversePatterns`) на «Anders Holmström — биография»
